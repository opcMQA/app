...
class TradeAggregator:
    ...
    async def add_trade(self, symbol, second, usd_size, is_buyer_maker):
        ...
        self.trade_bucket[trade_key] = self.trade_bucket.get(trade_key, 0) + usd_size

    async def check_and_print_trades(self):
        timestamp_now = datetime.utcnow().strftime('%H:%M:%S')
        deletions = []

        for trade_key, usd_size in self.trade_bucket.items():
    ...
    async with connect(uri) as websocket:
        while True:
            try:
                ...
                cprint(f"Error in binance_trade_stream for {symbol}: {e}", "red")
